...
**Current executable:** build **#743** / `63e23f22d46c5075597f6f882ac2e039b6fd8b93`, [full public CI green on attempt 2](https://github.com/Vorith03/VulkanMod/actions/runs/36229149682). Install its `-all.jar`; keep `config/fml.toml` -> `earlyWindowControl = false` and the established renderer-replacement baseline. First check Creative item icons and the third-person player, then whether an advancement plaque still triggers the Iceberg framebuffer layout crash. If those clear, continue with moving Create/Flywheel visuals, Create UI, representative effects, and a real portal. The user deferred `F3+T`, forced dirty hybrid rebuild, and world re-entry for this pass. Use `docs/CREATE_CHRONICLES_RETEST_2026-09-25.md` for the short current test sequence.

**Evidence distinction:** the user's build #728 RX 6900 XT run retained both actual PureBDcraft packs, activated Vulkan on RADV, and exercised the GPU terrain path before Distant Horizons 3.2.0-b called `GL11.glGetInteger(GL_FRAMEBUFFER_BINDING)` at Forge AFTER_LEVEL and aborted. Build #742 cancels only that OpenGL-only DH callback; CI #742 validates the compatibility target without loading DH's Forge proxy too early. The private two-pack CI passed on an earlier head and the user's #728 run confirmed pack retention; neither proves the current full-pack visuals. DH LOD rendering remains unavailable under Vulkan.

**2026-09-26 full-pack #742 result:** the user's RX 6900 XT log confirms the world continued past DH AFTER_LEVEL, entered Creative, and rendered world blocks. The user could not see block/item icons in the creative inventory or their character; these are open Phase 4 visual defects. The modpack's isolated Everycomp missing-model warnings do not explain this broad symptom. At the Ice Bucket Challenge advancement, Advancement Plaques 1.6.9 / Iceberg 1.1.25 invoked `GuiHelper.innerBlit` and the first fatal error was `IllegalStateException: Unexpected texture layout before shader read: 2` in `Pipeline$DescriptorSets.updateDescriptorSet`. Layout 2 is `VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL`: an ordinary GUI draw sampled a framebuffer attachment without the primary-frame transition that post effects already use. #743 prepares attachment samplers on ordinary `BufferUploader.drawWithShader` and VBO draws before binding their pipeline; CI is green, but full-pack GPU validation and the independent missing-icons/player issue remain unverified. No reload or re-entry testing is requested for this pass.

### Current compatibility matrix

| Component or path | Established evidence | Current Phase 4 status |
| --- | --- | --- |
| VulkanMod on RX 6900 XT / RADV | #742 reached Creative with world blocks visible; inventory icons/player invisible | PASS for world entry; GUI/entity visuals defective |
...
| Distant Horizons 3.2.0-b | #728 AFTER_LEVEL raw-GL abort; #742 RX world continuation past that site | PASS for guarded callback; LOD draw remains suppressed |
