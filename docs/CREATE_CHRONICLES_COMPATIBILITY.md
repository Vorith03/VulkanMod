...
**2026-09-26 full-pack #742 result:** the user's RX 6900 XT log confirms the world continued past DH AFTER_LEVEL, entered Creative, and rendered world blocks. The user could not see block/item icons in the creative inventory or their character; these are open Phase 4 visual defects. The modpack's isolated Everycomp missing-model warnings do not explain this broad symptom. At the Ice Bucket Challenge advancement, Advancement Plaques 1.6.9 / Iceberg 1.1.25 invoked `GuiHelper.innerBlit` and the first fatal error was `IllegalStateException: Unexpected texture layout before shader read: 2` in `Pipeline$DescriptorSets.updateDescriptorSet`. Layout 2 is `VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL`: an ordinary GUI draw sampled a framebuffer attachment without the primary-frame transition that post effects already use. The pending code fix prepares attachment samplers on ordinary `BufferUploader.drawWithShader` and VBO draws before binding their pipeline; full-pack GPU validation and the independent missing-icons/player issue remain unverified. No reload or re-entry testing is requested for this pass.
