...
import net.vulkanmod.render.chunk.voxel.GpuSparseLightingGpuSmokeTest;
...
public final class SectionVoxelSmokeTest {
    ...
    public static void verify() {
        ...
        try {
            ...
            var task = new ChunkTask.BuildTask(section, null, origin(section), false, dispatcher);
            ...
            task = new ChunkTask.BuildTask(section, null, origin(section), false, dispatcher);
            dispatcher.scheduleSectionUpdate(task, section, new EnumMap<>(TerrainRenderType.class),
                    () -> section.publishVoxels(snapshot, generation));
            task.cancel();
            dispatcher.uploadAllPendingUploads();
            ...
            dispatcher.scheduleSectionUpdate(new ChunkTask.BuildTask(section, null, origin(section), false, dispatcher), section,
            ...
            GpuSparseLightingGpuSmokeTest.verify();
    ...
    private static BlockPos origin(RenderSection section) {
        return new BlockPos(section.xOffset(), section.yOffset(), section.zOffset()).immutable();
    }
