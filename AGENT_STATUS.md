...
- Latest executable checkpoint is `4abc931918a2e43ff76f5428ce855bfcd3309537` (`compat: bridge Create stencil GL calls`), fully validated by CI #715.
...
- CI #715 is fully green on one coherent executable tree: build/distributable, both Vulkan startup smokes, persistent GPU-indirect, vanilla post/depth chains, screenshot readback, FTB Library, Pick Up Notifier, Immersive Portals 3.0.7, Distant Horizons 3.2.0-b, Crash Assistant, Chat Heads, Flywheel, and an exact Create 0.5.1.j stencil fixture all pass. The completed audit repairs remain closed; the post-audit Create follow-up now additionally covers real off-screen Vulkan depth/stencil attachments, separate depth-sampling vs depth/stencil attachment views, Vulkan stencil pipeline/clear state, and Create's raw `GL_STENCIL_TEST` toggles through a call-site mixin.
...
- The build #711 log also contained repeated non-fatal-at-the-time `IllegalArgumentException: last char is not ;` shader-conversion exceptions during Immersive Portals shader rebuilding. They did not terminate that run, but remain a known runtime signal to inspect before or during the next full-pack test rather than assuming they are harmless.
...
1. Use CI #715 / `4abc931918a2` for the next RX 6900 XT / RADV Create Chronicles correctness run; earlier builds do not contain the complete Create stencil repair.
...
3. Confirm startup gets past Create's `CustomRenderTarget.enableStencil()` path and retain any Immersive Portals shader-conversion exception if it still occurs. Capture only evidence that distinguishes correctness/fallback outcomes: visible terrain/portal artifacts, crashes, relevant logs, and whether dirty rebuilds preserve complete geometry.
