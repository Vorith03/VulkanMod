...
- Latest executable checkpoint is `b9910cfb067504ec766fbb2488cc379e1d98e5c8` (`compat: bind IP clipping for aliased terrain shaders`), fully validated by CI #720.
...
- CI #720 is fully green on one coherent executable tree: build/distributable, both Vulkan startup smokes, persistent GPU-indirect, vanilla post/depth chains, screenshot readback, FTB Library, Pick Up Notifier, Immersive Portals 3.0.7, Distant Horizons 3.2.0-b, Crash Assistant, Chat Heads, Flywheel, and the exact Create 0.5.1.j stencil fixture all pass. The Immersive Portals smoke additionally constructs a Forge-namespaced shader that aliases vanilla `rendertype_cutout`, proves the transformed Vulkan pipeline receives the authoritative terrain clip-plane binding, and logs `VULKANMOD_IP_ALIASED_TERRAIN_CLIP_OK`.
...
1. Use CI #720 / `b9910cfb067504ec766fbb2488cc379e1d98e5c8` for the next RX 6900 XT / RADV Create Chronicles correctness run; it includes both the complete Create stencil repair and the full-pack shader-parser/aliased-IP fixes exposed by build #711.
2. Use the existing full-pack test boundary rather than recollecting settled evidence: enable the experimental REPLACE + APPEND path, enter the target world, exercise a real Immersive Portals portal, and force at least one dirty mixed-section rebuild.
3. Confirm startup gets past both prior #711 blockers: Create's `CustomRenderTarget.enableStencil()` path and Twilight Forest's `red_thread` registration. The old `last char is not ;` parser exception and `Immersive Portals clipping uniform was not attached to shader rendertype_cutout` should both be absent. Capture only evidence that distinguishes correctness/fallback outcomes: visible terrain/portal artifacts, crashes, relevant logs, and whether dirty rebuilds preserve complete geometry.
