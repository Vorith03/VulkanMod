...
- Verified runtime source: `29df210a6d73141e069e7aa90cdddc4a4506b146`; CI **#308** build and all configured smoke-test steps passed (job 103508179666), including the new packed texture GPU pixel/readback test under synchronization validation.
...
- Active work: current Create/Flywheel visual coverage and the prerequisite full-pack resource-reload/world-reentry retest. The #304 memory guard failure remains the latest full-pack runtime evidence. Build #308 introduces packed texture staging; whole-process RSS savings and full-pack reload survival are not yet measured.
- Commits: `29df210a6d73141e069e7aa90cdddc4a4506b146` reduces texture staging allocation/copying and adds CPU/GPU regression coverage; the following documentation checkpoint records the result. See the source-memory section below. No safety limits were changed.
...
- CI #308 build, packaging and every smoke-test step passed. The packed texture layout test also passed in CI; GPU readback verifies full/offset rectangles, nonzero source position and packed staging size in both unbatched and batched submission. The screenshot script rejects validation errors and synchronization hazards. No full-pack RSS reduction, reload survival or performance improvement is claimed yet. This fixes avoidable upload allocation/copying; the #304 mip-generation-time RSS failure may have additional causes.
