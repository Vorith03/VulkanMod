...
- Latest executable checkpoint is `5964641c5428dedb3df02e3f3a31bc949b12f7eb` (`compat: bind IP clipping for aliased model-view shaders`), fully validated by CI #721.
...
- CI #721 is fully green on one coherent executable tree: build/distributable, both Vulkan startup smokes, persistent GPU-indirect, vanilla post/depth chains, screenshot readback, FTB Library, Pick Up Notifier, Immersive Portals 3.0.7, Distant Horizons 3.2.0-b, Crash Assistant, Chat Heads, Flywheel, and the exact Create 0.5.1.j stencil fixture all pass. The Immersive Portals smoke covers both alias classes: `rendertype_cutout` receives the authoritative pre-model-view terrain clip plane (`VULKANMOD_IP_ALIASED_TERRAIN_CLIP_OK`), while `rendertype_entity_translucent` receives IP's runtime-selected model-view clip state (`VULKANMOD_IP_ALIASED_MODEL_VIEW_CLIP_OK`).
...
- The RX 6900 XT full-pack build #720 run confirmed the #711 fixes moved the failure frontier: Vulkan activated, the old parser exception and Create stencil abort did not recur, and resource loading proceeded until Alex's Caves registered `alexscaves:rendertype_sepia`, whose vertex program aliases IP-transformed `rendertype_entity_translucent`. That exposed the same ShaderInstance-name/program-name mismatch in IP's entity/particle transform group. `5964641c5428` mirrors IP's own runtime state machine for aliased model-view shaders: after-model-view clipping during entity/projection rendering, pre-model-view during portal weather, disabled otherwise; unknown transform groups remain fail-closed. CI #721 covers the exact `rendertype_entity_translucent` alias shape.
...
Compatibility is no longer an independent workstream. The relevant Forge 1.20.1 compatibility stack is integrated into the current `forge-1.20.1` executable head `5964641c5428` and validated by CI #721.
...
1. Use CI #721 / `5964641c5428dedb3df02e3f3a31bc949b12f7eb` for the next RX 6900 XT / RADV Create Chronicles correctness run; build #720 is superseded because it exposed the Alex's Caves/IP model-view alias blocker.
2. Keep the same four experimental REPLACE + APPEND flags. Startup should now pass Create's old stencil site, Twilight Forest's `red_thread -> rendertype_cutout`, and Alex's Caves `rendertype_sepia -> rendertype_entity_translucent`; the old parser and both missing-clipping-uniform signatures should be absent.
3. If startup reaches the title screen, enter the target world, exercise a real Immersive Portals portal, and force at least one dirty mixed-section rebuild. Capture only evidence that distinguishes correctness/fallback outcomes: visible terrain/entity/portal artifacts, crashes, relevant logs, and whether dirty rebuilds preserve complete geometry.
